...
class FlexibleAgent002(CaptureAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        FlexibleAgent002.previousFoodToDefend = self.getFoodYouAreDefending(gameState)
        ...
        if not self.attack:
            ...
            for x in range(self.xDim):
                for y in range(self.yDim):
                    if currentFoodToDefend[x][y] != FlexibleAgent002.previousFoodToDefend[x][y]:
                        diff.append((x,y))
            FlexibleAgent002.previousFoodToDefend = currentFoodToDefend
            if diff != []:
                ...
                if actionToTake != None:
                    self.chaseCounter = 7
                    FlexibleAgent002.rememberedPosition = (int(posToGo[0]), int(posToGo[1]))
                    return actionToTake
            ...
            myPos = gameState.getAgentPosition(self.index)
            ennemy_pos = FlexibleAgent002.rememberedPosition
